          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">最近错误</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="item in reviewPlan.items" :key="item.char">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ item.char }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.shengmu }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.yunmu }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.errorCount }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ formatDate(item.lastError) }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.reviewCount }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const getErrorCount = (value, type) => {
  ...
  if (type === 'shengmu') {
    return items.filter(item => item.shengmu === value)
      .reduce((sum, item) => sum + item.errorCount, 0)
  } else if (type === 'yunmu') {
    return items.filter(item => item.yunmu === value)
      .reduce((sum, item) => sum + item.errorCount, 0)
  } else {
